...
import usuarios.medicos.Expediente;
import usuarios.medicos.Rol;
import usuarios.medicos.Usuario;

/**
 * Clase que representa el Hospital. Gestiona pacientes, médicos, consultas y
 * más.
 *
 * @author ameti
 */
public class Hospital {
    ...
    public Hospital() {
        Administrador administrador = new Administrador("admin1", "Juan", "Perez", LocalDate.now(), "435", "1234", Rol.ADMIN, 20000, 20, "JUAN12323ML");
    ...
    public void mostrarInfoMedico(Medico medico) {
        System.out.println("\nInformación del Médico:");
        System.out.println("Id: " + medico.getId());
        System.out.println("Nombre: " + medico.getNombre());
        System.out.println("Apellido: " + medico.getApellidos());
    ...
    public void mostrarInfoAdmin(Administrador admin) {
        System.out.println("\nInformación del Médico:");
        System.out.println("Id: " + admin.getId());
        System.out.println("Nombre: " + admin.getNombre());
        System.out.println("Apellido: " + admin.getApellidos());
        System.out.println("Fecha de nacimiento: " + admin.getFechaNacimiento());
        System.out.println("Teléfono: " + admin.getTelefono());
        System.out.println("RFC: " + admin.getRFCAdmin());
    }

    public void mostrarConsultasProgramadas(String idMedico) {
        boolean existenConsultas = false;

        System.out.println("\nConsultas del Médico con ID: " + idMedico);
        for (Consulta consulta : listaConsultas) {
            if (consulta.getMedico().getId().equals(idMedico)) {
                System.out.println(consulta.mostrarInformacion()); // Mostrar la información de la consulta
                existenConsultas = true;
            }
        }
        if (!existenConsultas) {
            System.out.println("El médico con ID " + idMedico + " no tiene consultas registradas.");
        }
    }
    ...
    public void generarExpedienteConsulta(String idConsulta, String idPaciente) {
        ...
        if (consulta == null) {
            System.out.println("No existe una consulta con el ID proporcionato");
            return;
        }
        Paciente paciente = this.obtenerPacientePorId(idPaciente);
        //si el paciente es null, no existe
        if (paciente == null) {
            System.out.println("No existe un paciente con el id proporcionado");
            return;
        }
        //En caso de que no entre a los IFs de arriba, actualiza el status de consulta

        consulta.setStatus(Status.TERMINADA);
        this.eliminarConsultaPorId(idConsulta);
        System.out.println("Ingresa las observaciones finales del a consulta: ");
    ...
    public boolean validarCorreoUnico(String correo) {
        ...
        for (Medico medico : listaMedicos) {
            if (medico.getEmail().equals(correo)) {
                return false;
            }
        }
